...
var _ = ginkgo.Describe("SystemReconciler.Reconcile", ginkgo.Label("integration"), func() {
	ginkgo.It("should reconcile OCP system", func() {
		...
		ocpClientMock.On("PutBundle", mock.Anything, &ocp.PutBundleRequest{
			...
			Revision: `$"data:{crypto.sha256(concat("", [input.sources["path-to-datasource"].sql.hash, input.sources["default-ocp-system"].sql.hash, input.sources["library1"].sql.hash]))},` +
				`git-sha:{input.sources["default-ocp-system"].git.commit},` +
				`libraries:{crypto.sha256(concat("", [input.sources["path-to-datasource"].git.commit, input.sources["default-ocp-system"].git.commit, input.sources["library1"].git.commit]))}"`,
		}).Return(nil).Times(3)
