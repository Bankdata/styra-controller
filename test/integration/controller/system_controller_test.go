...
var _ = ginkgo.Describe("SystemReconciler.ReconcileOCPSystem", ginkgo.Label("integration"), func() {
	ginkgo.It("should reconcile", func() {
		...
		//Called in CreateIfNotExist
		ocpClientMock.On("GetSource", mock.Anything, "path/datasource").Return(&ocp.GetSourceResponse{},
			httperror.NewHTTPError(http.StatusNotFound, "")).Once()

		// Called in CreateIfNotExist
		ocpClientMock.On("PutSource", mock.Anything, "path/datasource", &ocp.PutSourceRequest{
			Name: "path/datasource",
		}).Return(&ocp.PutSourceResponse{}, nil).Once()

		// Called in reconcileSystemSource
		ocpClientMock.On("PutSource", mock.Anything, "default-ocp-system-cluster1", &ocp.PutSourceRequest{
			Name: "default-ocp-system-cluster1",
		}).Return(&ocp.PutSourceResponse{}, nil).Once()

		// Note: new reconciles when updating secret and configmaps
		...
		ocpClientMock.On("PutBundle", mock.Anything, &ocp.PutBundleRequest{
			...
			ObjectStorage: ocp.ObjectStorage{
				AmazonS3: &ocp.AmazonS3{
					Bucket:      "test-bucket",
					Key:         "bundles/default-ocp-system-cluster1/bundle.tar.gz",
					Region:      "eu-west-1",
					URL:         "storage-url",
					Credentials: "ocp-config",
				},
			},
		...
		gomega.Eventually(func() bool {
			...
			fmt.Println("getSource", getSource)
			fmt.Println("putSource", putSource)
			fmt.Println("putBundle", putBundle)
