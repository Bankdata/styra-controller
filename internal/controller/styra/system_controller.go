...
func (r *SystemReconciler) reconcilek8sOPASecret(
	...
	log logr.Logger,
	system *v1beta1.System,
	s3Credentials configv2alpha2.AWSCredentials,
	secretName string,
	SecretKeyNameAccessID string,
	...
	if err := r.Get(ctx, nsName, &s); err != nil {
		if k8serrors.IsNotFound(err) {
			...
			s.Data = map[string][]byte{
				SecretKeyNameAccessID:  []byte(s3Credentials.AccessKeyID),
				SecretKeyNameSecretKey: []byte(s3Credentials.SecretAccessKey),
				SecretKeyNameRegion:    []byte(s3Credentials.Region),
			}
	...
	if (string(s.Data[SecretKeyNameAccessID]) != s3Credentials.AccessKeyID) ||
		(string(s.Data[SecretKeyNameSecretKey]) != s3Credentials.SecretAccessKey) ||
		(string(s.Data[SecretKeyNameRegion]) != s3Credentials.Region) {
...
func (r *SystemReconciler) reconcileS3Credentials(
	...
	SecretKeyNameAccessID string,
	SecretKeyNameSecretKey string,
) (configv2alpha2.AWSCredentials, ctrl.Result, error) {
	s3Credentials := configv2alpha2.AWSCredentials{}
